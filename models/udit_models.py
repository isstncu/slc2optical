...
class ConditionModule(nn.Module):
    def __init__(self,in_channels, out_channels,latent_size=32, block_out_channels = [16, 32, 64, 96, 192, 384]):
        super(ConditionModule, self).__init__()
        self.conv_in = nn.Conv3d(in_channels, block_out_channels[0], kernel_size=(3, 3, 3), padding=(1, 1, 1))
        self.blocks = nn.ModuleList([])
        for i in range(0, len(block_out_channels) - 1):
            self.blocks.append(
                nn.Conv3d(block_out_channels[i], block_out_channels[i], kernel_size=3, padding=1, stride=(1, 1, 1)))
            self.blocks.append(
                nn.Conv3d(block_out_channels[i], block_out_channels[i + 1], kernel_size=3, padding=1, stride=(1, 2, 2)))

        self.conv_out = nn.Conv3d(block_out_channels[-1], out_channels, kernel_size=(3, 3, 3), padding=(1, 1, 1))
    ...
    def forward(self, x):
        ...
        for index, block in enumerate(self.blocks):
            if x.shape[-1] <= self.latent_size and index%2 == 1:
...
class OpticalConditionModule(nn.Module):
    def __init__(self,in_channels, out_channels,latent_size=32, block_out_channels = [16, 32, 64, 96, 192, 384]):
        super(OpticalConditionModule, self).__init__()
        self.conv_in = nn.Conv2d(in_channels, block_out_channels[0], kernel_size=(3, 3), padding=(1, 1))
        self.blocks = nn.ModuleList([])
        for i in range(0, len(block_out_channels) - 1):
            self.blocks.append(
                nn.Conv2d(block_out_channels[i], block_out_channels[i], kernel_size=3, padding=1, stride=(1, 1)))
            self.blocks.append(
                nn.Conv2d(block_out_channels[i], block_out_channels[i + 1], kernel_size=3, padding=1, stride=(2, 2)))

        self.conv_out = nn.Conv2d(block_out_channels[-1], out_channels, kernel_size=(3, 3), padding=(1, 1))
        self.latent_size = latent_size
    ...
    def forward(self, x):
        ...
        for index, block in enumerate(self.blocks):
            if x.shape[-1] <= self.latent_size and index%2 == 1:
                mid = rearrange(x, "(b f) c h w -> b f c h w",f=f)
                middle_feature.append(mid)
            x = block(x)
            x = F.silu(x)
        x = self.conv_out(x)
...
class InterpolationBlock(nn.Module):
    def __init__(self,in_channels, out_channels, num_frames=1):
        super().__init__()
        self.num_frames = num_frames
        self.conv_in = nn.Conv3d(in_channels=in_channels,out_channels=in_channels,kernel_size=(3,3,3),stride=1,padding=1)
        ...
        self.norm_list = nn.ModuleList([
            LayerNorm3d(in_channels, affine=False, eps=1e-6)
            for _ in range(num_frames)])
        self.conv_out = nn.Conv3d(in_channels=in_channels*num_frames,out_channels=out_channels,kernel_size=(3,3,3),stride=1,padding=1)
    def forward(self, x, refer_opt):
        b, f, c, h, w = refer_opt.shape
        assert self.num_frames == f, "Number of frames not equal to number of frames in InterpolationBlock!"
        combine_list = []
        x = rearrange(x,"(b f) c h w -> b f c h w",b=b)
        x = rearrange(x,"b f c h w ->b c f h w")
        x = self.conv_in(x)
        for i in range(self.num_frames):
            refer = refer_opt[:, i:i+1, :, :, :]
            refer = rearrange(refer,"b f c h w ->b c f h w")
            block = self.branch_list[i]
            norm_block = self.norm_list[i]
            interpolation_x = x + refer
            interpolation_x = block(F.silu(norm_block(interpolation_x)))
            # interpolation_x = F.silu(interpolation_x)
            combine_list.append(interpolation_x)
        out = torch.cat(combine_list, dim=1)
        out = self.conv_out(F.silu(out))
        out = rearrange(out, "b c f h w -> (b f) c h w")
        return out

class CHBlock(nn.Module):
    def __init__(self,in_channels, out_channels,num_frames=4):
        super().__init__()
        self.conv1 = nn.Conv3d(in_channels=in_channels,out_channels=out_channels,kernel_size=(3, 3, 3), stride=(1,1,1),padding=(1,1,1))
        self.norm1 = LayerNorm3d(out_channels, affine=False, eps=1e-6)
        self.conv2 = nn.Conv3d(in_channels=out_channels,out_channels=out_channels,kernel_size=(3, 3, 3), stride=(1,1,1), padding=(1,1,1))
        self.norm2 = LayerNorm3d(out_channels, affine=False, eps=1e-6)
        self.dropout = nn.Dropout(p=0.1,inplace=True)
        self.num_frames = num_frames

    def forward(self, x):
        x = rearrange(x,"(b f) c h w -> b c f h w", f=self.num_frames)
        # b, c, f, h, w = x.shape
        # x = rearrange(x,"b f c h w ->b c f h w", f=self.num_frames)
        r = self.conv1(F.relu(self.norm1(x)))
        r = self.dropout(r)
        r = self.conv2(F.relu(self.norm2(r)))
        out = x+r
        out = rearrange(out,"b c f h w -> (b f) c h w")
        return out
class U_DiTDeCodeBlock(nn.Module):
    """
    A IPT block with adaptive layer norm zero (adaLN-Zero) conIPTioning.
    """

    def __init__(self, input_size, hidden_size, num_heads, mlp_ratio=4.0, down_factor=2, rep=1, ffn_type='rep',
                 mixture=False, expert_num=2,
                 **kwargs):
        '''
        Args:
        dw_expand: For NAFNet
        ...
        if mixture:
            self.interpolation_list = nn.ModuleList([
                InterpolationBlock(hidden_size, hidden_size, i+1) for i in range(expert_num)
            ])
        self.direct_block = CHBlock(hidden_size,hidden_size)
    ...
    def forward(self, x, c, refer_opt=None):
        if self.mixture and refer_opt is not None:
            b, f, c1, h, w = refer_opt.shape
            block = self.interpolation_list[f-1]
            x = block(x,refer_opt)
        else:
            x = self.direct_block(x)
...
class U_DiT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.condition_endoder = ConditionModule(in_channels=3, out_channels=hidden_size*4)
        self.optical_encoder = OpticalConditionModule(in_channels=3, out_channels=hidden_size*4)
        ...
        self.feature_fusion1 = nn.Conv2d(hidden_size*2,hidden_size,kernel_size=3,stride=1,padding=1,bias=True)
        ...
        self.feature_fusion2 = nn.Conv2d(hidden_size * 4, hidden_size*2, kernel_size=3, stride=1, padding=1, bias=True)
        ...
        # ])
        self.latent = nn.ModuleList([
            U_DiTDeCodeBlock(input_size // 4, hidden_size * 4, num_heads, mlp_ratio=mlp_ratio, down_factor=down_factor[2],
                       rep=rep, ffn_type=ffn_type, mixture=False, **kwargs)
            if latent_index%2==0 else
            U_DiTDeCodeBlock(input_size // 4, hidden_size * 4, num_heads, mlp_ratio=mlp_ratio,
                             down_factor=down_factor[2],
                             rep=rep, ffn_type=ffn_type, mixture=True, **kwargs)
            for latent_index in range(depth[2])
        ])


        self.feature_fusion3 = nn.Conv2d(hidden_size * 8, hidden_size*4, kernel_size=3, stride=1, padding=1, bias=True)
        ...
        self.decoder_level_2 = nn.ModuleList([
            U_DiTDeCodeBlock(input_size//2, hidden_size*2, num_heads, mlp_ratio=mlp_ratio,
                             down_factor=down_factor[3], rep=rep, ffn_type=ffn_type,mixture=False,
                             expert_num=expert_num, **kwargs)
            if decode_index % 2 == 0 else
            U_DiTDeCodeBlock(input_size // 2, hidden_size * 2, num_heads, mlp_ratio=mlp_ratio,
                             down_factor=down_factor[3], rep=rep, ffn_type=ffn_type,mixture=True,
                             **kwargs)
            for decode_index in range(depth[3])
        ])
        # self.decoder_level_2 = nn.ModuleList([
        ...
        #     U_DiTDeCodeBlock(input_size, hidden_size * 2, num_heads, mlp_ratio=mlp_ratio,
        #                      down_factor=down_factor[4], rep=rep, ffn_type=ffn_type, mixture=True,
        #                      expert_num=expert_num, **kwargs)
        #     if decode_index%2 == 0 else
        #     U_DiTDeCodeBlock(input_size, hidden_size * 2, num_heads, mlp_ratio=mlp_ratio,
        #                      down_factor=down_factor[4], rep=rep, ffn_type=ffn_type, mixture=False,
        #                      expert_num=expert_num, **kwargs)
        #     for decode_index in range(depth[4])
        # ])
        self.decoder_level_1 = nn.ModuleList([
    ...
    def forward(self, x, t, y, refer_opt=None):
        """
        ...
        # if refer_opt != None:
        #     print(f"refer_opt: {refer_opt.shape}")
        x = rearrange(x, "b f c h w -> (b f) c h w")
        ...
        t1 = t1.unsqueeze(1)
        t1 = repeat(t1, "b f d -> b (f n) d", n=4)
        t1 = rearrange(t1, "b f d -> (b f) d")

        condition_feature = self.condition_endoder(y)
        if refer_opt != None:
            refer_opt = self.optical_encoder(refer_opt)
        ...
        t2 = t2.unsqueeze(1)
        t2 = repeat(t2, "b f d -> b (f n) d", n=4)
        t2 = rearrange(t2, "b f d -> (b f) d")
        ...
        t3 = t3.unsqueeze(1)
        t3 = repeat(t3, "b f d -> b (f n) d", n=4)
        t3 = rearrange(t3, "b f d -> (b f) d")
        ...
        # encoder_1
        out_enc_level1 = x
        condition_enc1 = rearrange(condition_feature[0],"b c f h w -> (b f) c h w")
        out_enc_level1 = torch.cat([out_enc_level1, condition_enc1], dim=1)
        out_enc_level1 = self.feature_fusion1(out_enc_level1)
        ...
        condition_enc2 = rearrange(condition_feature[1], "b c f h w -> (b f) c h w")
        out_enc_level2 = torch.cat([out_enc_level2, condition_enc2], dim=1)
        out_enc_level2 = self.feature_fusion2(out_enc_level2)
        ...
        condition_enc3 = rearrange(condition_feature[2], "b c f h w -> (b f) c h w")
        latent = torch.cat([latent, condition_enc3], dim=1)
        latent = self.feature_fusion3(latent)
        for index, block in enumerate(self.latent):
            if index % 2 != 0 and refer_opt != None:
                latent = block(latent, c3, refer_opt[2])
            else:
        ...
        for index, block in enumerate(self.decoder_level_2):
            if index % 2 != 0 and refer_opt != None:
                out_dec_level2 = block(out_dec_level2, c2, refer_opt[1])
            else:
        ...
        for index, block in enumerate(self.decoder_level_1):
            # if index%2 == 0:
        ...
        x = rearrange(x, "(b f) c h w -> b f c h w",f=4)
        return x
...
if __name__=="__main__":
    ...
    model = U_DiT_S()
    # model.load_state_dict(torch.load('pth/U-DiT-S-400k.pt'))

    model.cuda()
    model.eval()

    inputs = torch.rand(2, 4, 4, 32, 32).cuda() #(B, F, C, H, W)
    t = torch.ones(2).int().cuda()
    y = torch.rand(2, 4, 3, 256, 256).cuda()    #(B, C, F, H, W)
    refer_opt = torch.rand(2, 1, 3, 256, 256).cuda()    #(B, F, C, H, W)

    model(inputs, t, y,refer_opt)
