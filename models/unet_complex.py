...
class DiffusionComplexUNet(nn.Module):
    def __init__(self, input_nc, output_nc):
        ...
        #                                stride=1,
        #                                padding=1)
        self.complexconv_in = ComplexConv2d(self.in_channels,  # 2
                                       self.ch,  # 128
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
    ...
    def upsample_concate(self, hs_fft):
        hs_fft_real = torch.real(torch.fft.ifftn(hs_fft.pop(), dim=(-2, -1)))
        return hs_fft_real
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = self.up[i_level].block[i_block](
                    # v3 ifftn转换
                    torch.cat([h, (hs.pop() + self.upsample_concate(hs_fft))], dim=1),
                    temb)  # pop删除并返回最后一个元素
