...
class WindowAttention3(nn.Module):
    ...
    def forward(self, x_diff, x_hr, x_lr, x_c, mask=None):
        ...
        """
        B_, N, C = x_diff.shape
        q = self.q(x_lr).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k = self.k(x_hr).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q2 = self.q2(x_lr).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k2 = self.k2(x_c).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        v = self.v(x_diff).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class SwinTransformerBlock3(nn.Module):
    ...
    def forward(self, x_diff, x_lr, x_hr, x_c):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(images[0], images[1], images[2], images[3], mask=self.attn_mask)  # nW*B, window_size*window_size, C
...
class BasicLayer3(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            SwinTransformerBlock3(dim=dim, input_resolution=input_resolution,
                                  num_heads=num_heads, window_size=window_size,
                                  shift_size=0 if (i % 2 == 0) else window_size // 2,
                                  mlp_ratio=mlp_ratio,
                                  qkv_bias=qkv_bias, qk_scale=qk_scale,
