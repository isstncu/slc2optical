...
class ComplexConv2Deffangle(nn.Module):
    def __init__(self, in_channels, out_channels, kern_size, stride, drop_prob=0.0):
        ...
        self.weight_matrix_rot1 = torch.nn.Parameter(torch.rand(in_channels, kern_size[0] * kern_size[1]),
                                                     requires_grad=True)
    ...
    def forward(self, x):
        ...
        temporal_buckets = nn.Unfold(kernel_size=self.kern_size, stride=self.stride)(x).view(x_shape[0], x_shape[1],
                                                                                             self.in_channels,
                                                                                             self.kern_size[0] *
                                                                                             self.kern_size[1], -1)
        ...
        out_rot = ((torch.sum(temporal_buckets_rot.permute(0, 3, 1, 2).contiguous().view(-1, tbr_shape[1], tbr_shape[
            2]) * weightNormalize1(self.weight_matrix_rot1), 2))).view(tbr_shape[0], tbr_shape[3],
                                                                       tbr_shape[1]).permute(0, 2, 1).contiguous()
        ...
        out_rot = (torch.sum(out_rot * weightNormalize1(self.weight_matrix_rot2), 2)).view(out_rot_shape[0], 1,
                                                                                           out_spatial_x, out_spatial_y,
                                                                                           self.out_channels).permute(0,
                                                                                                                      1,
                                                                                                                      4,
                                                                                                                      2,
                                                                                                                      3).contiguous()
        ...
        out_abs = ((torch.sum(temporal_buckets_abs.permute(0, 3, 1, 2).contiguous().view(-1, tba_shape[1], tba_shape[
            2]) * weightNormalize(self.weight_matrix_rot1, self.drop_prob), 2))).view(tba_shape[0], tba_shape[3],
                                                                                      tba_shape[1]).permute(0, 2,
                                                                                                            1).contiguous()
        ...
        out_abs = torch.exp(torch.sum(out_abs * weightNormalize(self.weight_matrix_rot2, self.drop_prob), 2)).view(
            out_abs_shape[0], 1, out_spatial_x, out_spatial_y, self.out_channels).permute(0, 1, 4, 2, 3).contiguous()
...
class ComplexLinearangle2Dmw_outfield(nn.Module):
    ...
    def forward(self, x):
        ...
        all_data = all_data.view(all_data_shape[0], all_data_shape[1] * all_data_shape[2] * all_data_shape[3],
                                 all_data_shape[4])
        ...
        dist_rot = self.SOmetric(all_data_rot.view(-1), M_rot.unsqueeze(1).repeat(1, all_shape[1]).view(-1)).view(
            all_shape[0], all_shape[1])
        dist_abs = self.P1metric(all_data_abs.view(-1), M_abs.unsqueeze(1).repeat(1, all_shape[1]).view(-1)).view(
            all_shape[0], all_shape[1])
...
class manifoldReLUv2angle(nn.Module):
    ...
    def forward(self, x):
        ...
        temp_rot = x[:, 0, ...]
        temp_abs = x[:, 1, ...]
        temp_rot_prod = (temp_rot.unsqueeze(1) * (weightNormalize2(self.weight_rot + eps)).unsqueeze(0).unsqueeze(
            3).unsqueeze(4).repeat(x_shape[0], 1, 1, x_shape[3], x_shape[4]))
        temp_abs = (temp_abs.unsqueeze(1) + (weightNormalize1(self.weight_abs + eps)).unsqueeze(0).unsqueeze(
            3).unsqueeze(4).repeat(x_shape[0], 1, 1, x_shape[3], x_shape[4]))
...
class ComplexConv2Deffgroup(nn.Module):
    ...
    def forward(self, x):
        ...
        temporal_buckets_rot = temporal_buckets_rot.permute(0, 3, 1, 2).contiguous().view(-1, tbr_shape0[1],
                                                                                          tbr_shape0[2])
        temporal_buckets_abs = temporal_buckets_abs.permute(0, 3, 1, 2).contiguous().view(-1, tbr_shape0[1],
                                                                                          tbr_shape0[2])
        ...
        if self.do_conv:
            in_rot = in_rot.view(tbr_shape0[0], out_spatial_x, out_spatial_y, -1).permute(0, 3, 1,
                                                                                          2).contiguous().unsqueeze(1)
            in_abs = in_abs.view(tbr_shape0[0], out_spatial_x, out_spatial_y, -1).permute(0, 3, 1,
                                                                                          2).contiguous().unsqueeze(1)
            ...
            in_rot = torch.mean(in_rot, 2).view(tbr_shape0[0], out_spatial_x, out_spatial_y, -1).permute(0, 3, 1,
                                                                                                         2).contiguous().unsqueeze(
                1)
            in_abs = torch.mean(in_abs, 2).view(tbr_shape0[0], out_spatial_x, out_spatial_y, -1).permute(0, 3, 1,
                                                                                                         2).contiguous().unsqueeze(
                1)
