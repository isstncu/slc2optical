...
class ResidualAttentionModel_92_32input_update(nn.Module):
    ...
    def __init__(self):
        ...
        self.residual_block4 = ResidualBlock(512, 1024, 2)  # 8*8
        self.residual_block5 = ResidualBlock(1024, 1024)  # 8*8
        self.residual_block6 = ResidualBlock(1024, 512)  # 8*8
        self.residual_block7 = ResidualBlock(512, 256)  # 4*4
        self.conv_layer = nn.Conv2d(in_channels=256, out_channels=4, kernel_size=1, stride=1, padding=0)
        self.mpool2 = nn.Sequential(
            nn.BatchNorm2d(4),
            nn.ReLU(inplace=True),
        ...
        self.decoder = nn.Sequential(
            # 第一次下采样：4x4 → 2x2
            nn.Conv2d(256, 128, kernel_size=3, stride=2, padding=1),  # [64, 128, 2, 2]
            nn.BatchNorm2d(128),
            nn.ReLU(),

            # 第二次下采样：2x2 → 1x1
            nn.Conv2d(128, 64, kernel_size=3, stride=2, padding=1),  # [64, 64, 1, 1]
            nn.BatchNorm2d(64),
            nn.ReLU(),

            # 调整通道数到目标维度
            nn.Conv2d(64, 4, kernel_size=1)  # [64, 4, 1, 1]
        )
        ...
        #     nn.ReLU(),
        #
        #     # Step 2: 高度从4→2，宽度保持4，继续减少通道
        #     nn.Conv2d(128, 64, kernel_size=3, stride=2, padding=1),
        #     nn.BatchNorm2d(64),
        #     nn.ReLU(),
        #
        #     # Step 3: 保持空间尺寸，进一步减少通道
        #     nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1),
        #     nn.BatchNorm2d(32),
        #     nn.ReLU(),
        #
        #     # Step 4: 宽度从2→1，通道数调整到目标4
        #     nn.Conv2d(32, 4, kernel_size=(1, 2), stride=(1, 2), padding=0),
        # )
        # self.ds_decoder2 = nn.Sequential(
        #     # 保持空间尺寸不变，逐步减少通道数
    ...
    def forward(self, x):
        ...
        # out = self.mpool1(out)
        # print(out.data)
        out = self.residual_block1(out) # torch.Size([64, 128, 32, 32])
        out = self.attention_module1(out) # torch.Size([64, 128, 32, 32])
        out = self.residual_block2(out) # # torch.Size([64, 256, 16, 16])
        out = self.attention_module2(out)
        out = self.attention_module2_2(out)
        out = self.residual_block3(out)
        # print(out.data)
        ...
        out = self.attention_module3_2(out)
        out = self.attention_module3_3(out)
        out = self.residual_block4(out)# torch.Size([64, 1024, 4, 4])
        out = self.residual_block5(out)# torch.Size([64, 1024, 4, 4])
        # print(out.shape)
        out = self.residual_block6(out) # torch.Size([64, 512, 4, 4])
        # print(out.shape)
        out = self.residual_block7(out) # torch.Size([64, 256, 4, 4])
        # print(out.shape)
        ##out = self.conv_layer(out) # torch.Size([64, 4, 4, 4])
        # print(out.shape)
        ##out = self.mpool2(out) # torch.Size([64, 4, 2, 1])
        # 只预测均值或方差
        out = self.decoder(out) # torch.Size([64, 4, 1, 1])
        # 预测均值和方差
        # out = self.decoder2(out)  # torch.Size([64, 4, 2, 1])
        # print(out.shape)
